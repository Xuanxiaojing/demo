...
let store = new Vuex.Store({ 
  state: {
    ...
    musicList:[/* {
      ...
      "songid":"14361",
      "singer":"王菲"
    } */], // 最近播放的音乐组成的播放列表，用于latalyList.vue组件的渲染
    lyricData:{}, // lyricData时一个对象，key值是歌曲的id，value值是歌词（转码后的）
    AllMusicList:[], // 所有音乐
    musicIndex:0,
  ...
  mutations: {
    ...
    changeListData(state,payload){
      ...
      // console.log(payload[0],11111111111111)
      // let bl = true;
      // state.listdata.forEach((item)=>{
        
      //   if(item.id === payload[0].id) return
      // })
      for(var i=0; i<state.listdata.length; i++){
        if(state.listdata[i].id === payload[0].id) return
      }
    ...
    setListDataMusic(state,payload){
      state.listdata.forEach(function(item1){                        
        if(item1.type === '音乐'){
            ...
            if(item1.music.includes((state.AllMusicList[musicIndex]))) return 
            item1.music.push(state.AllMusicList[musicIndex]);                                     
            musicIndex++;
        }
      })
    },
    ...
    changeListIsplay(state,payload){
      ...
      Vue.set(state.listdata[payload.index],'isPlay',payload.isPlay)
    },
    ...
    setAllMusicList(state,payload){
      ...
      // 每次都
      // console.log(state.AllMusicList.length,'lllllllllllllllllllllllll')
      ...
      //     }
      //   })
      //   // console.log(state.listdata,'ddddddddddddd') 
      // }
    },
